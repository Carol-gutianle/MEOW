...
from meow.dataset import TextDatasetQA, custom_data_collator, custom_data_collator_with_indices
from meow.utils import get_model_identifiers_from_yaml, get_model_utility, get_forget_quality, get_batch_loss
...
@hydra.main(version_base=None, config_path="config", config_name="eval_phi")
def main(cfg):
    ...
    # 添加single_evaluate
    folder = f'{cfg.method_name}_{cfg.model_family}_{cfg.split}'
    ...
    robot = Robot()
    robot.post_message(f'''eval实验完成! 模型路径:{cfg.model_path}, 方法名称{cfg.method_name}! Model Utility: {model_utility['Model Utility']}, Forget Quality: {forget_quality['Forget Quality']}!''')
...
def run_generation(cfg, batch, model, tokenizer):
    ...
    split_symbol = " [/INST]" if 'llama' in cfg.model_family else 'Answer: '
    ground_truth = [s.split(split_symbol)[1] for s in input_strings]
    input_strings = [s.split(split_symbol)[0] for s in input_strings]
    #add ["/INST "] to the end of each string
    if 'llama' in cfg.model_family:
